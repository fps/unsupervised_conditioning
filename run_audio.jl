...
num_latent_variables = length(max_frequencies)
...
opt = Flux.setup(Flux.AdamW(0.0001), [model, latent_params])
...
for m in 1:5000;
    ...
    for n in 1:50; 
        ...
        the_loss, the_grads = Flux.withgradient([model, latent_params]) do params
            ...
            loss(params[1], cat(x, latent, dims=2), y) # + 0.001f0 * sum((eye .- g(cov(latent))).^2) # + 0.00001f0 * (latent[:,1]' * latent[:,2])^2.0f0 # + 0.00001f0 * Statistics.mean(latent).^2.0f0 # + 0.00000001 * sum((1.0f0 .- Statistics.var(latent, dims=2))).^2# + 0.00001 * sum(abs.(latent)) # #  # + 10.0f0 * Statistics.mean(diff(latent, dims=1).^2)# 0.01f0 * Statistics.mean(latent.^2) #  + 1.0f0 * (1.0f0 - Statistics.var(latent)).^2 + Statistics.mean(latent.^2) + 0.1f0*(latent[:,1]' * latent[:,2])^2.0f0 # + 0.1f0 * (1.0f0 - Statistics.var(latent)).^2 + 0.1f0*(latent[:,1]' * latent[:,2])^2.0f0
            # + sum(([1 0] .- latent[1,:]).^2)
